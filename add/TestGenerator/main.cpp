...
void printLine(std::ostream &out, unsigned long long ll_x, unsigned long long ll_y, bool add)
{
...
int exponent(unsigned long long x)
{
	const unsigned long long EXP_MASK = 0x7FF0000000000000ULL;
	const unsigned EXP_SHIFT = 52;
	return (x & EXP_MASK) >> EXP_SHIFT;
}

void fillFile(std::ofstream &testfile, bool add)
{
	const unsigned MAX_EXP_DIFF = 53;
	unsigned long long ll_x, ll_y;
    for(int fk_i = FK_ZERO; fk_i < FK_LAST; fk_i++)
    {
        for(int fk_j = FK_ZERO; fk_j < FK_LAST; fk_j++)
        {
            ...
            ll_x = floatKind2int64(fk_x);
            ll_y = floatKind2int64(fk_y);
...
int main()
{
    ...
    testfile.open ("test_file_add.txt");
	fillFile(testfile, true);
    testfile.close();
    testfile.open ("test_file_sub.txt");
	fillFile(testfile, false);
    testfile.close();
